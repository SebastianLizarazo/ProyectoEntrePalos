...
CREATE TABLE IF NOT EXISTS `bdEntrePalos`.`Municipio` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `Nombre` VARCHAR(90) NOT NULL,
  `Depatamentos_id` BIGINT UNSIGNED NOT NULL,
  `Estado` ENUM('Activo', 'Inactivo') NOT NULL,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC),
  INDEX `fk_Municipio_Depatamentos1_idx` (`Depatamentos_id` ASC),
  CONSTRAINT `fk_Municipio_Depatamentos1`
    FOREIGN KEY (`Depatamentos_id`)
...
CREATE TABLE IF NOT EXISTS `bdEntrePalos`.`DetalleOferta` (
  ...
  INDEX `fk_Producto_has_Ofertas_Ofertas1_idx` (`Ofertas_id` ASC),
